...
xString Index::CreateName(const xString& tableName)
{
	xString ret = L"IX_" + tableName;
...
xString Index::CreateColumnsText()
{
	xString ret;
...
bool Index::DependsOn(const xString& columnName)
{
...
ColumnRef Table::FindColumn(const xString& columnName)
{
	auto findIt = std::find_if(_columns.begin(), _columns.end(),
		[&](const ColumnRef& column) { return column->_name == columnName; });
...
xString Procedure::GenerateParamString()
{
	xString str;
...
xString Helpers::Format(const WCHAR* format, ...)
{
	WCHAR buf[4096];
...
xString Helpers::DataType2String(DataType type)
{
	switch (type)
	{
...
xString Helpers::RemoveWhiteSpace(const xString& str)
{
	xString ret = str;
